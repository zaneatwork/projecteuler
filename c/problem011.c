...
int maxAdjacentProductAtIndex(int grid[], int gridSize, int i) {
  int maxAdjacentProduct = 0;

  // Find left-to-right product if we can
  if (i % 20 == 0 || (i % 20 % 17 > 0 && i % 20 % 18 > 0 && i % 20 % 19 > 0)) {
    maxAdjacentProduct = grid[i] * grid[i + 1] * grid[i + 2] * grid[i + 3];
  }
  ...
  // Find left-to-right diagonal product if we can
  if (i < gridSize - 63 &&
...
int main() {
  int numberGrid[GRID_SIZE];
  for (int x = 0, y = 0; x < sizeof(CHAR_GRID) - 2; x += 3, y++) {
    numberGrid[y] = (CHAR_GRID[x] - '0') * 10 + (CHAR_GRID[x + 1] - '0');
  }
